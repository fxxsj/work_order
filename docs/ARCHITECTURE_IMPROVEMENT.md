...
- [x] 前端：迁移 API 模块到工厂（`apps/web/src/api/` 业务模块均已使用 `createCrudApi/createApiWithActions`，并移除重复 `PaginatedResult` 定义）
...
- [ ] 后端：继续迁移剩余 ViewSet（目标：大多数简单 CRUD 继承 `BaseViewSet`；进行中：`materials.py(PurchaseOrder/PurchaseOrderItem/PurchaseReceiveRecord)` + `assets.py(Artwork/Die/FoilingPlate/EmbossingPlate)` + `work_orders.py(WorkOrder/DraftTask)` + `work_order_tasks.py(WorkOrderTask)` + `work_order_processes.py(WorkOrderProcess)`）
...
- [x] Release / Tag 产物（`v*` tag：上传 `WorkOrder-<tag>-web.zip` + Desktop bundles/installers + Android APK/AAB 到 GitHub Release）
...
- [x] 库存管理 TODO：补齐与测试（入库审核生成 `ProductStock` 批次 + 出库审核扣减库存 + 发货签收照片上传）
- [x] 财务模块 TODO：对账单 / 期初余额等（客户/供应商对账单生成已补齐；供应商付款记录尚未建模，贷方暂为 0）
