...
- [x] 前端：迁移部分 API 模块到工厂（已完成：`customers/departments/materials/products/suppliers/payments/invoices/tasks/workorders(list)/processes/catalog/productGroups`）
...
- [x] 后端：落地 `BaseViewSet` 并迁移部分 ViewSet（已完成：`base.py/products.py/materials.py` 的核心 CRUD + `system.py(TaskAssignmentRule)`）
