...
- [x] 财务模块界面（成本中心/成本项/生产成本/对账单等基础列表与操作：`apps/web/src/views/CostCenterListView.vue` 等）
- [x] 高级任务管理 UI（批量完成/取消/分派、分派历史：`apps/web/src/views/TaskListView.vue`）
- [x] 导出功能 UI（施工单/任务列表 Excel 导出：`apps/web/src/views/WorkOrderListView.vue` / `apps/web/src/views/TaskListView.vue`）
...
- [x] 桌面端：托盘（System Tray：显示/隐藏/退出 + 跳转客户端下载）
- [x] 桌面端：原生通知（Web Notification：应用后台时弹系统通知）
- [x] 桌面端：更新入口（托盘菜单跳转 `#/download` 客户端下载页；Release 产物链路已打通）
- [x] 移动端：相机/文件上传（签收照片：`receiver_signature`）
- [x] 移动端：扫码（`/scan`：基于 `BarcodeDetector` 的轻量扫码页）
- [x] 移动端：弱网体验（online/offline 提示 + online 时重连通知 WS）
- [ ] 移动端：推送通知（需要 Capacitor 插件 + 服务端推送通道，暂未落地）
...
- 高级任务管理 UI 与导出 UI 已在 Web 端落地（`TaskListView`/`WorkOrderListView`），不再属于“缺失功能”。
- 财务/成本基础 UI 已落地（成本中心/成本项/生产成本/对账单），但“供应商付款记录（贷方）”等建模仍有空缺（详见 `docs/ARCHITECTURE_IMPROVEMENT.md`）。
...
| **Web 平台** | 92% | 施工单同步预览/多级审批高级动作（上报/报表）等仍缺少完整 UI |
...
| 多级审批 | ✅ 完整（含初始化命令与最小测试） | ⚠️ 部分（已提供工作流管理 + 审批待办） | 继续补齐上报/报表/规则 UI |
| 监控/分析 | ✅ 部分 | ⚠️ 部分（基础仪表板已落地） | 中优先级 |
...
- ❌ 施工单同步预览 UI
- ⚠️ 多级审批界面（已提供：`/approval-workflows`、`/approval-steps`；上报/报表/规则仍缺失）
- ⚠️ 性能监控仪表板（基础版已落地：`/monitoring`）
- ❌ 财务分析/报表（非基础 CRUD 的分析看板）
...
| 模块 | 后端 API | 前端视图 | 前端 API | 完成度 |
|------|----------|----------|----------|--------|
| **财务/成本** | ✅ 100% | ⚠️ 70% | ⚠️ 70% | 70% |
| **多级审批** | ✅ 100% | ⚠️ 35% | ⚠️ 35% | 35% |
| **监控/分析** | ⚠️ 50% | ⚠️ 40% | ⚠️ 40% | 40% |
| **报表/导出** | ✅ 100% | ⚠️ 70% | ⚠️ 70% | 70% |
...
1. **完成缺失的前端功能**
   - [x] 财务模块界面（成本中心/成本项/生产成本/对账单等基础列表与操作）
   - [x] 高级任务管理 UI（批量完成/取消/分派、分派历史）
...
1. **高级功能**
   - [ ] 多级审批前端（已落地：工作流管理 + 审批待办；待补齐：上报/报表/规则等）
   - [x] 监控仪表板（基础版）
   - [ ] 高级报表
