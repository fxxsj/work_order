...
| **Web 平台** | 90% | 多级审批执行流/施工单同步预览等仍缺少完整 UI |
...
| 多级审批 | ✅ 完整（含初始化命令与最小测试） | ⚠️ 部分（已提供工作流管理 UI） | 继续补齐执行流 UI |
...
- ❌ 施工单同步预览 UI
- ⚠️ 多级审批界面（已提供工作流管理：`/approval-workflows`；执行流仍缺失）
...
1. **高级功能**
   - [ ] 多级审批前端（已落地：工作流管理；待补齐：审批待办/处理/上报等执行流）
   - [x] 监控仪表板（基础版）
   - [ ] 高级报表
