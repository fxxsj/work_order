...
| 布局架构 | ✅ 已落地 | 7/10 | 已引入 `AuthedLayout`（侧边栏/抽屉导航 + 全局 Header）并逐步迁移页面级 `PageLayout` |
| 移动端适配 | ⚠️ 改善 | 5/10 | 已补 `viewport-fit=cover`；`施工单/任务列表` 小屏降级为卡片列表 |
| UI完善度 | ⚠️ 基础 | 5/10 | 无图标、无主题切换 |
| 代码质量 | ⚠️ 一般 | 6/10 | 已开始抽公共布局/样式；Element Plus 仍为全量引入 |

### 核心问题

1. **导航覆盖仍不完整** - 侧边栏仅覆盖常用入口，其余页面仍依赖工作台按钮进入
2. **页面级结构仍有重复** - 仍有不少页面沿用旧的 `.page/.bar` 结构，需继续迁移到 `PageLayout`
3. **移动端适配仍不均衡** - 已优化核心列表页，但其它表格页仍可能在小屏产生横向滚动
...
├── layouts/                # 布局（AuthedLayout）
├── components/             # 通用组件（PageLayout/SideNav 等）
├── composables/            # 组合式能力（useBreakpoints）
...
**特点**：
- 已引入统一的认证布局 `AuthedLayout`（全局 Header + 桌面侧边栏 + 移动端抽屉菜单）
- 页面内部仍普遍保留“返回 + 标题 + 操作区”的页面级 bar（正在逐步迁移到 `PageLayout`）
...
| 组件 | 主流方案 | 本项目 | 影响 |
|------|---------|-------|------|
| **Sidebar** | 有 | ✅ 有（覆盖常用入口） | 导航效率提升，但覆盖面仍需扩展 |
| **Header** | 有 | ✅ 有（用户/通知/退出） | 全局入口统一 |
...
**说明**：
- 已增加 `viewport-fit=cover`（适配刘海屏/安全区域）
- 不建议通过 `user-scalable=no` / `maximum-scale=1.0` 禁止缩放（影响可访问性与可用性）

### 响应式设计现状

**现状**：已引入基础响应式能力（全局 `@media (max-width: 768px)` + `useBreakpoints()`），但覆盖范围仍主要集中在核心列表页与通用布局。

```css
/* 例：ClientDownloadView.vue:232 */
...
1. **断点体系仍偏弱** - 当前主要以 768px 为主进行“手机/非手机”二分
...
App.vue
└── router-view
    ├── LoginView.vue / ClientDownloadView.vue（公开路由）
    └── AuthedLayout.vue（认证路由）
        ├── SideNav（桌面）/ DrawerNav（移动）
        ├── Header（用户/通知/退出）
        └── router-view（业务页面）
```
...
| # | 问题 | 影响 | 优先级 |
|---|------|------|-------|
| 1 | 导航覆盖不完整 | 仍需依赖工作台按钮进入部分页面 | 高 |
| 2 | 无面包屑导航 | 层级感知弱 | 中 |
| 3 | 无多标签页 | 任务切换不便 | 中 |
| 4 | 页面级 bar 仍分散 | 迁移未完成，仍存在重复结构/样式 | 中 |

### 移动端适配问题

| # | 问题 | 影响 | 优先级 |
|---|------|------|-------|
| 1 | 响应式覆盖不均 | 核心列表页已优化，其它表格页仍可能难用 | 高 |
| 2 | 表格未适配 | 小屏幕难用 | 高 |
| 3 | 操作按钮未优化 | 点击困难 | 中 |
| 4 | 移动端专用布局不足 | 目前主要依赖抽屉菜单，缺少更轻量的移动导航模式 | 中 |
