...
- [x] 图标体系（基础）：`apps/web/package.json`（`@element-plus/icons-vue`）+ `AuthedLayout/MobileTabBar` 使用示例
...
| 图标体系 | ElementUI 内置 | ✅ 已引入（基础版） | 可读性/可扫读性提升（关键入口） | 按需扩展：SideNav、列表页关键操作、空状态等 |
