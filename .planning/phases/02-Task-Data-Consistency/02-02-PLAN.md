...
<task type="auto">
  ...
  <action>
    Add bulk operation endpoints to WorkOrderTaskViewSet (or create new ViewSet if it doesn't exist):

    1. Find WorkOrderTaskViewSet or create new ViewSet in views/work_orders.py
  ...
  <verify>grep -n "def bulk_update\|def bulk_delete" backend/workorder/views/work_orders.py</verify>
...
1. Backend: grep shows bulk_update and bulk_delete action methods in views
