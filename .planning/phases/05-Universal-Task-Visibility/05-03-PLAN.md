...
must_haves:
  ...
  artifacts:
    - path: "backend/workorder/views/work_order_tasks/task_bulk.py"
      provides: "TaskBulkMixin with batch_delete endpoint"
  ...
  key_links:
    ...
    - from: "frontend/src/views/task/components/BatchAssignDialog.vue"
      ...
      via: "dialog integration (Plan 02)"
      pattern: "handleConfirmBatchAssign"
---

<objective>
Implement backend batch_delete endpoint and frontend batch API methods (batchAssign, batchComplete, batchCancel, batchDelete). Create BatchAssignDialog component for batch assignment UI integration.

Purpose: Complete PAGE-03 requirement for batch operations on task list. This plan provides the API layer used by Plan 02's batch UI components.

Output: Enhanced TaskBulkMixin with batch_delete, BatchAssignDialog component, and frontend API methods.

Note: This plan runs in Wave 1 (parallel with Plan 01) because backend API and frontend API methods are independent of Plan 02's UI work. Plan 02 depends on this plan for the actual API method implementations.
</objective>
...
1. Test batch delete endpoint:
   - POST to /api/workorder-tasks/batch-delete/ with task_ids
   - Draft tasks deleted successfully
   - Non-draft tasks return error in failed_tasks list

2. Test batch assign API method:
   - Call batchAssign with task_ids, department, operator
   - Tasks assigned successfully
   - Verify department and operator updated

3. Test batch complete API method:
   - Call batchComplete with task_ids
   - Tasks status changed to completed

4. Test batch cancel API method:
   - Call batchCancel with task_ids and cancellation_reason
   - Tasks status changed to cancelled

5. Test BatchAssignDialog component:
   - Dialog renders with correct props
   - Department selection loads operators
   - Form validation works
   - Confirm event emits correct data structure

Note: TaskList.vue integration of BatchAssignDialog and batch operations is handled in Plan 02.
</verification>

<success_criteria>
1. batch_delete endpoint added to TaskBulkMixin with draft-only validation
2. BatchAssignDialog component created with department/operator selection
3. Frontend API methods for all batch operations present
4. Batch operations API endpoints return partial success/failure responses
5. BatchAssignDialog loads operators when department changes
</success_criteria>

<output>
After completion, create `.planning/phases/05-Universal-Task-Visibility/05-03-SUMMARY.md` with:
- batch_delete endpoint implementation details
- BatchAssignDialog component details
- API method additions
- Batch operation API testing results
</output>
