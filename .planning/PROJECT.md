...
- [ ] **SEC-01**: 升级 axios 到 1.7.4+ (修复 CVE-2023-45857 CSRF 漏洞)
- [ ] **SEC-02**: 升级 ws (jest 依赖) 到 8.17.1+ (修复 DoS 漏洞)
- [ ] **CODE-01**: 拆分 Detail.vue (3508行 → 多个子组件)
- [ ] **CODE-02**: 拆分 WorkOrderForm.vue (1472行 → 多个子组件)
- [ ] **CODE-03**: 创建 src/constants/ 目录统一管理状态常量
- [ ] **CODE-04**: 清理后端测试文件中的 print() 语句 (120+ 处)
- [ ] **TEST-01**: 创建后端单元测试框架 (当前覆盖率 0%)
- [ ] **TEST-02**: 补充后端核心模型测试
- [ ] **CLEAN-01**: 清理前端 TODO 注释 (3 处)
- [ ] **CLEAN-02**: 清理事件监听器内存泄漏风险
...
**v1.1 待办**：
根据代码质量审查报告（2026-02-02）：
- 安全漏洞：axios 1.6.2 (CSRF)、ws (DoS)
- 代码重复：后端批量操作方法重复、前端大文件需拆分
- 调试信息：后端 120+ 处 print() 需清理
- 测试覆盖：后端 0%，需建立测试体系
- 规范问题：魔数硬编码、内存泄漏风险
...
| v1.1 安全加固 | 修复 axios/ws 安全漏洞 | ⏳ 待实施 |
| v1.1 代码重构 | 拆分大文件，创建常量目录 | ⏳ 待实施 |
| v1.1 测试覆盖 | 建立后端单元测试体系 | ⏳ 待实施 |

---
*Last updated: 2026-02-02 after code quality review and v1.1 milestone creation*
