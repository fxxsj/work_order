    ...
    try:
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT 1 as active_connections FROM pg_stat_activity WHERE state = 'active'"
                )
...
class AlertChecker:
    ...
    def check_alerts(self, metrics):
        ...
        if (
            metrics
            and metrics.get("memory", {}).get("percent", 0) > self.thresholds["memory"]
        ):
            alerts.append(
                {
                    "type": "system",
        ...
        if (
            metrics
            and metrics.get("disk", {}).get("percent", 0) > self.thresholds["disk"]
        ):
            alerts.append(
                {
                    "type": "system",
