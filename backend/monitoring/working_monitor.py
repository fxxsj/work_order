        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if duration > 5.0:  # 超过5秒的请求
                    logger.warning(
                        f"Slow operation detected: {metric_name} took {duration:.2f}s"
                    )
...
class DatabaseMonitor:
    ...
    def record_query(self, duration: float, query_type: str = "unknown"):
        ...
        if duration > 2.0:  # 慢查询阈值
            self.slow_queries.append(
                {"duration": duration, "type": query_type, "timestamp": datetime.now()}
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_query_time": (
                statistics.mean(self.query_times) if self.query_times else 0
            ),
            "recent_slow_queries": [
                {
                    "duration": q["duration"],
...
class SystemHealthMonitor:
    ...
    def check_memory_usage(self) -> Dict[str, Any]:
        """检查内存使用（简化版本）"""
        return {"status": "ok", "message": "Memory check not implemented"}  # 简化实现
