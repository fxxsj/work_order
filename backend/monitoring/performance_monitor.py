...
class AlertManager:
    ...
    def check_system_alerts(self, metrics):
        ...
        if (
            metrics
            and metrics.get("memory", {}).get("percent", 0) > rules["memory_threshold"]
        ):
            alerts.append(
                {
                    "type": "system",
        ...
        if (
            metrics
            and metrics.get("disk", {}).get("percent", 0) > rules["disk_threshold"]
        ):
            alerts.append(
                {
                    "type": "system",
    ...
    def check_application_alerts(self, metrics):
        ...
        if (
            metrics
            and metrics.get("avg_response_time", 0) > rules["response_time_threshold"]
        ):
            alerts.append(
                {
                    "type": "application",
    ...
    def check_database_alerts(self, metrics):
        ...
        if (
            metrics
            and metrics.get("active_connections", 0)
            > rules["database_connections_threshold"]
        ):
            alerts.append(
                {
                    "type": "database",
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--output-file", type=str, help="Output metrics to JSON file"
        )
    ...
    def handle(self, *args, **options):
        ...
        if options.get("check_alerts"):
            ...
            if application_metrics:
                alerts.extend(
                    alert_manager.check_application_alerts(application_metrics)
                )
...
class BusinessMetricsCollector:
    ...
    def get_user_activity_metrics(self):
        ...
        try:
            ...
            recent_active_users = User.objects.filter(last_login__gte=yesterday).count()
