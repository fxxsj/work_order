...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        if not username or not password:
            return Response(
                {"error": "请提供用户名和密码"}, status=status.HTTP_400_BAD_REQUEST
            )
...
def change_password(request):
    ...
    if new_password != confirm_password:
        return Response(
            {"error": "新密码和确认密码不一致"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if len(new_password) < 6:
        return Response(
            {"error": "新密码长度至少为6位"}, status=status.HTTP_400_BAD_REQUEST
        )
