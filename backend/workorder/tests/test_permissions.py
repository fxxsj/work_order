...
from ..models import Customer, Department, Process, Product, WorkOrder
...
class WorkOrderTaskPermissionTest(APITestCaseMixin, TestCase):
    ...
    def test_operator_can_only_update_own_tasks(self):
        ...
        response = self.api_post(
            f"/api/workorder-tasks/{task1.id}/update_quantity/",
            data,
            user=self.operator1,
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.api_post(
            f"/api/workorder-tasks/{task2.id}/update_quantity/",
            data,
            user=self.operator1,
        )
    ...
    def test_supervisor_can_update_department_tasks(self):
        """测试生产主管可以更新部门内所有任务"""
        from ..models import UserProfile, WorkOrderProcess, WorkOrderTask
...
class ApprovalPermissionTest(APITestCaseMixin, TestCase):
    ...
    def setUp(self):
        ...
        # 添加产品和工序以满足审核条件
        from ..models import Artwork, Die, WorkOrderProcess, WorkOrderProduct
    ...
    def test_salesperson_can_approve_own_customer_orders(self):
        ...
        # 审核施工单（使用正确的参数名）
        response = self.api_post(
            f"/api/workorders/{self.wo1.id}/approve/",
    ...
    def test_rejection_requires_reason(self):
        ...
        response = self.api_post(
            f"/api/workorders/{self.wo1.id}/approve/",
            {"approval_status": "rejected", "rejection_reason": "信息不完整"},
            user=self.salesperson1,
        )
...
class APIAuthenticationTest(TestCase):
    ...
    def test_logout(self):
        ...
        # 登出（使用 token 认证）
        response = self.client.post(
            "/api/auth/logout/", HTTP_AUTHORIZATION=f"Token {token}"
        )
