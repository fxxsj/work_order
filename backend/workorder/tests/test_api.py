...
from ..models import Customer, Process, Product, WorkOrder
...
class WorkOrderAPITest(APITestCaseMixin, TestCase):
    ...
    def test_list_workorders(self):
        """测试获取施工单列表"""
        # 创建施工单
        TestDataFactory.create_workorder(customer=self.customer, creator=self.user)
    ...
    def test_update_workorder(self):
        ...
        response = self.api_patch(
            f"/api/workorders/{work_order.id}/", data, user=self.user
        )
    ...
    def test_search_by_order_number(self):
        ...
        response = self.api_get(
            f"/api/workorders/?search={work_order.order_number}", user=self.user
        )
...
class WorkOrderProcessAPITest(APITestCaseMixin, TestCase):
    ...
    def test_list_processes(self):
        ...
        response = self.api_get(
            f"/api/workorder-processes/?work_order={self.work_order.id}", user=self.user
        )
    ...
    def test_start_process(self):
        ...
        response = self.api_post(
            f"/api/workorder-processes/{wo_process.id}/start/", user=self.user
        )
    ...
    def test_batch_start_processes(self):
        ...
        response = self.api_post(
            "/api/workorder-processes/batch_start/", data, user=self.user
        )
...
class WorkOrderTaskAPITest(APITestCaseMixin, TestCase):
    ...
    def test_list_tasks(self):
        ...
        response = self.api_get(
            f"/api/workorder-tasks/?work_order_process={wo_process.id}", user=self.user
        )
    ...
    def test_update_task_quantity(self):
        ...
        response = self.api_post(
            f"/api/workorder-tasks/{task.id}/update_quantity/", data, user=self.user
        )
    ...
    def test_complete_task(self):
        ...
        response = self.api_post(
            f"/api/workorder-tasks/{task.id}/complete/", user=self.user
        )
    ...
    def test_assign_task(self):
        """测试分派任务"""
        from ..models import Department, WorkOrderProcess, WorkOrderTask
        ...
        data = {"assigned_department": department.id, "assigned_operator": self.user.id}

        response = self.api_post(
            f"/api/workorder-tasks/{task.id}/assign/", data, user=self.user
        )
