...
class DepartmentModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.root_dept = Department.objects.create(
            name="总部", code="headquarters", sort_order=0, is_active=True
        )
...
class DepartmentSerializerTest(TestCase):
    ...
    def test_code_too_short(self):
        """测试部门编码太短"""
        data = {"name": "测试部门", "code": "a", "sort_order": 0, "is_active": True}
        serializer = DepartmentSerializer(data=data)
        self.assertFalse(serializer.is_valid())
        self.assertIn("code", serializer.errors)
    ...
    def test_name_validation(self):
        """测试部门名称验证"""
        # 名称太短
        data = {"name": "测", "code": "test_dept", "sort_order": 0, "is_active": True}
        serializer = DepartmentSerializer(data=data)
        self.assertFalse(serializer.is_valid())
...
class DepartmentAPITest(APITestCase):
    ...
    def test_create_department(self):
        ...
        self.client.force_authenticate(user=self.admin_user)
        url = reverse("department-list")
        data = {"name": "财务部", "code": "finance", "sort_order": 2, "is_active": True}
