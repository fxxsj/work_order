...
class ProcessSerializerTest(TestCase):
    ...
    def test_validate_code_valid(self):
        ...
        for code in valid_codes:
            serializer = ProcessSerializer(data={"code": code, "name": "测试工序"})
            self.assertTrue(
                serializer.is_valid(), f"编码 {code} 应该是有效的: {serializer.errors}"
            )
    ...
    def test_validate_builtin_code_change(self):
        """测试内置工序的编码不可修改"""
        process = Process.objects.create(name="印刷", code="PRINT", is_builtin=True)
...
class ProcessAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        self.process = Process.objects.create(name="印刷", code="PRINT", sort_order=1)
    ...
    def test_batch_update_active(self):
        """测试批量更新启用状态"""
        process2 = Process.objects.create(name="烫金", code="FOIL", is_active=True)
    ...
    def test_batch_disable_builtin_process(self):
        ...
        url = reverse("process-batch-update-active")
        data = {"ids": [self.process.id, builtin_process.id], "is_active": False}
        response = self.client.post(url, data, format="json")
