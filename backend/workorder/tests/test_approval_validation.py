...
class ApprovalValidationBaseTest(TestCase):
    ...
    def setUp(self):
        ...
        self.salesperson = User.objects.create_user(
            username="salesperson", password="testpass123", email="sales@example.com"
        )

        self.creator = User.objects.create_user(
            username="creator", password="testpass123", email="creator@example.com"
        )
        ...
        self.process_cut, _ = Process.objects.get_or_create(
            code="CUT", defaults={"name": "开料", "is_parallel": False}
        )

        # 创建版
        self.artwork = Artwork.objects.create(
            base_code="ART202401001", version=1, name="测试图稿", confirmed=False
        )
        ...
        # 创建刀模
        self.die = Die.objects.create(name="测试刀模", code="DIE001", size="100x100mm")
...
class PlateProcessMatchTest(ApprovalValidationBaseTest):
    ...
    def test_artwork_required_when_process_requires_artwork(self):
        ...
        WorkOrderProcess.objects.create(
            work_order=work_order, process=self.process_ctp, sequence=10  # 需要图稿
        )
...
class ProcessSequenceValidationTest(ApprovalValidationBaseTest):
    ...
    def test_ctp_before_prt(self):
        ...
        WorkOrderProcess.objects.create(
            work_order=work_order, process=self.process_prt, sequence=10  # 印刷在前
        )
    ...
    def test_ctp_after_prt(self):
        ...
        # 制版在前（正确）
        WorkOrderProcess.objects.create(
            work_order=work_order, process=self.process_ctp, sequence=10  # 制版在前
        )

        WorkOrderProcess.objects.create(
            work_order=work_order, process=self.process_prt, sequence=20  # 印刷在后
        )
    ...
    def test_cut_after_prt(self):
        ...
        # 开料在前（正确）
        WorkOrderProcess.objects.create(
            work_order=work_order, process=self.process_cut, sequence=10  # 开料在前
        )
...
class IntegrationTest(ApprovalValidationBaseTest):
    ...
    def test_valid_work_order_passes_all_validations(self):
        ...
        WorkOrderMaterial.objects.create(
            work_order=work_order, material=material_no_cutting, need_cutting=False
        )
