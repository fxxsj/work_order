...
def export_work_orders(queryset, filename=None):
    ...
    for row_num, work_order in enumerate(queryset, 2):
        ...
        ws.cell(
            row=row_num,
            column=4,
            value=work_order.created_by.username if work_order.created_by else "",
        )
        ws.cell(
            row=row_num,
            column=5,
...
def export_tasks(queryset, filename=None):
    ...
    for row_num, task in enumerate(queryset, 2):
        ...
        ws.cell(
            row=row_num,
            column=5,
            value=task.assigned_department.name if task.assigned_department else "",
        )
        ws.cell(
            row=row_num,
            column=6,
            value=task.assigned_operator.username if task.assigned_operator else "",
        )
