...
class SkillProfile(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="skill_profile"
    )

    # 核心技能领域
    technical_skills = models.JSONField(
        "技术技能", default=list, help_text="掌握的技术栈"
    )
    ...
    task_completion_rate = models.FloatField(
        "任务完成率", default=0.0, help_text="最近30天任务完成率"
    )
    avg_completion_time = models.FloatField(
        "平均完成时间(小时)", default=0.0, help_text="平均任务完成时间"
    )
    error_rate = models.FloatField("错误率", default=0.0, help_text="错误率")
    quality_score = models.FloatField(
        "质量评分", default=0.0, help_text="质量评分(0-10)"
    )

    # 工作偏好
    preferred_work_types = models.JSONField("偏好的工作类型", default=list)
    work_capacity = models.IntegerField(
        "工作容量", default=5, help_text="同时处理的最大任务数"
    )
...
class SmartAssignmentService:
    ...
    def calculate_skill_match_score(
        user_skills: List[str], task_requirements: List[str]
    ) -> float:
        """计算技能匹配度"""
    ...
    def calculate_priority_score(
        user, task_priority: str, deadline_days: int = None
    ) -> float:
        """计算优先级评分"""
        user_profile = SmartAssignmentService.get_skill_profile(user)
        if not user_profile:
    ...
    @staticmethod
    def get_optimal_operator(
        user, task_requirements: Dict[str, None]
    ) -> Optional[User]:
        """获取最佳操作员选择"""
        user_profile = SmartAssignmentService.get_skill_profile(user)
        if not user_profile:
        ...
        available_users = User.objects.filter(
            is_active=True,
            skill_profile__technical_skills__contains=any(
                skill.lower() for skill in task_requirements if skill
            ),
        )
        ...
        for user in available_users:
            score = SmartAssignmentService.calculate_user_match_score(
        ...
        if candidates_scores:
            return max(
                range(len(candidates_scores)), key=lambda i: candidates_scores[i]
            )
        return None
    ...
    def calculate_user_match_score(user, task_requirements: Dict[str, None]) -> float:
        ...
        score = SmartAssignmentService.calculate_skill_match_score(
            user_profile.technical_skills or [], task_requirements or []
        )

        # 考虑工作负载
        current_tasks = TaskPerformanceData.get_user_task_count(user)
        workload_factor = SmartAssignmentService.calculate_workload_factor(
            user, current_tasks
        )
    ...
    def calculate_team_balance_score(team_users: List[User]) -> Dict[str, float]:
        """计算团队平衡性评分"""
        skill_levels = [
            user.skill_profile.skill_level
...
class LearningSystem:
    ...
    def record_performance(
        ...
        logger.info(
            f"记录用户 {user_id} 的任务 {task_id} 性能数据: "
            f"成功={success}, "
    ...
    def update_skill_success_rate(
        self, user_id: int, skill_type: str, success_rate: float
    ):
        """更新技能成功率"""
        if user_id not in self.user_performance:
            self.user_performance[user_id] = TaskPerformanceData()
        ...
        if skill_type in self.user_performance[user_id].skill_success_rates:
            self.user_performance[user_id].skill_success_rates[
                skill_type
            ] = success_rate
    ...
    def get_system_recommendations(self) -> Dict[str, Any]:
        ...
        all_users = User.objects.filter(
            is_active=True, skill_profile__isnull=False
        ...
        for user in all_users:
            ...
            if perf_summary:
                ...
                total_performance.avg_completion_time += perf_summary[
                    "avg_completion_time"
                ]
        ...
        workload_scores = []
        for user in all_users:
            workload_score = SmartAssignmentService.calculate_workload_factor(
                user, TaskPerformanceData.get_user_task_count(user)
            )
