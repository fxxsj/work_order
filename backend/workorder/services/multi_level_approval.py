...
class ApprovalWorkflow(models.Model):
    ...
    min_amount = models.DecimalField(
        "最小金额", max_digits=12, decimal_places=2, null=True, blank=True
    )
    max_amount = models.DecimalField(
        "最大金额", max_digits=12, decimal_places=2, null=True, blank=True
    )
    priority_filter = models.JSONField(
        "优先级过滤", default=list, help_text="适用的优先级"
    )
...
class ApprovalStep(models.Model):
    ...
    work_order = models.ForeignKey(
        "WorkOrder", on_delete=models.CASCADE, related_name="approval_steps"
    )
...
class ApprovalRule(models.Model):
    ...
    workflow = models.ForeignKey(
        ApprovalWorkflow, on_delete=models.CASCADE, related_name="rules"
    )
...
class ApprovalEscalation(models.Model):
    ...
    work_order = models.ForeignKey(
        "WorkOrder", on_delete=models.CASCADE, related_name="escalations"
    )
...
class MultiLevelApprovalService:
    ...
    def create_approval_workflow(work_order, creator=None) -> ApprovalWorkflow:
        ...
        workflow = ApprovalWorkflow.objects.filter(
            workflow_type=workflow_type, is_active=True
        ).first()

        if not workflow:
            # 创建默认工作流
            workflow = MultiLevelApprovalService.create_default_workflow(
                workflow_type, creator
            )
    ...
    def start_approval_process(work_order, creator=None) -> List[ApprovalStep]:
        """启动审核流程"""
        # 确定工作流
        workflow = MultiLevelApprovalService.create_approval_workflow(
            work_order, creator
        )
        ...
        # 创建通知
        MultiLevelApprovalService.create_approval_notifications(
            work_order, approval_steps
        )

        return approval_steps

    @staticmethod
    def find_step_assignee(
        work_order, step_config: Dict, creator=None
    ) -> Optional[User]:
        """查找步骤执行人"""
        role_filter = step_config.get("role_filter", [])
        ...
        if work_order.customer and work_order.customer.salesperson:
            if work_order.customer.salesperson.groups.filter(
                name__in=role_filter
            ).exists():
    ...
    def complete_approval_step(
        step, decision: str, comments: str = "", user=None
    ) -> bool:
        """完成审核步骤"""
    ...
    def create_final_notification(work_order, decision: str):
        ...
        participants = User.objects.filter(
            approval_steps__work_order=work_order, approval_steps__status="completed"
        ).distinct()
...
class UrgentOrderService:
    ...
    def send_urgent_notification(work_order):
        ...
        managers = User.objects.filter(
            groups__name__in=["主管", "经理"], is_active=True
        )
    ...
    def get_urgent_orders() -> List[Dict]:
        ...
        urgent_orders = (
            WorkOrder.objects.filter(
                priority="urgent", approval_status__in=["pending", "approved"]
            )
            .prefetch_related(
                "customer", "order_processes__process", "products__product"
            )
            .order_by("-created_at")
        )
    ...
    def calculate_urgency_level(work_order) -> str:
        ...
        if work_order.delivery_date:
            days_until_delivery = (
                work_order.delivery_date - timezone.now().date()
            ).days
