...
class WorkOrderBusinessService:
    ...
    def create_workorder(data: Dict[str, Any], created_by: User) -> WorkOrder:
        ...
        if "processes" in data:
            WorkOrderBusinessService._create_workorder_processes(
                workorder, data["processes"]
            )

        # 创建施工单产品
        if "products" in data:
            WorkOrderBusinessService._create_workorder_products(
                workorder, data["products"]
            )

        # 创建施工单物料
        if "materials" in data:
            WorkOrderBusinessService._create_workorder_materials(
                workorder, data["materials"]
            )
    ...
    def update_workorder(
        workorder: WorkOrder, data: Dict[str, Any], updated_by: User
    ) -> WorkOrder:
        """
        更新施工单的业务逻辑
        ...
        # 更新关联数据
        if "processes" in data:
            WorkOrderBusinessService._update_workorder_processes(
                workorder, data["processes"]
            )

        if "products" in data:
            WorkOrderBusinessService._update_workorder_products(
                workorder, data["products"]
            )

        if "materials" in data:
            WorkOrderBusinessService._update_workorder_materials(
                workorder, data["materials"]
            )
    ...
    def approve_workorder(
        workorder: WorkOrder, approved_by: User, comments: str = ""
    ) -> bool:
        """
        审核施工单
    ...
    @staticmethod
    def reject_workorder(
        workorder: WorkOrder, rejected_by: User, comments: str = ""
    ) -> bool:
        """
        拒绝施工单
    ...
    def complete_workorder(workorder: WorkOrder, completed_by: User) -> bool:
        ...
        incomplete_processes = WorkOrderProcess.objects.filter(
            work_order=workorder, status__in=["pending", "in_progress"]
        ).exists()
    ...
    @staticmethod
    def _create_workorder_processes(
        workorder: WorkOrder, processes_data: List[Dict[str, Any]]
    ) -> None:
        """创建施工单工序"""
        for i, process_data in enumerate(processes_data, 1):
            ...
            try:
                ...
                WorkOrderProcess.objects.create(
                    ...
                    process=process,
                    order=i,
                    estimated_hours=process_data.get(
                        "estimated_hours", process.standard_hours
                    ),
                    status="pending",
                )
    ...
    @staticmethod
    def _create_workorder_products(
        workorder: WorkOrder, products_data: List[Dict[str, Any]]
    ) -> None:
        """创建施工单产品"""
    ...
    @staticmethod
    def _create_workorder_materials(
        workorder: WorkOrder, materials_data: List[Dict[str, Any]]
    ) -> None:
        """创建施工单物料"""
    ...
    @staticmethod
    def _update_workorder_processes(
        workorder: WorkOrder, processes_data: List[Dict[str, Any]]
    ) -> None:
        """更新施工单工序"""
        # 删除现有工序
    ...
    @staticmethod
    def _update_workorder_products(
        workorder: WorkOrder, products_data: List[Dict[str, Any]]
    ) -> None:
        """更新施工单产品"""
        # 删除现有产品
    ...
    @staticmethod
    def _update_workorder_materials(
        workorder: WorkOrder, materials_data: List[Dict[str, Any]]
    ) -> None:
        """更新施工单物料"""
        # 删除现有物料
...
class TaskBusinessService:
    ...
    def _check_and_start_next_process(completed_task: WorkOrderTask) -> None:
        ...
        current_process = WorkOrderProcess.objects.filter(
            work_order=workorder, process=completed_task.process
        ).first()
        ...
        next_process = WorkOrderProcess.objects.filter(
            work_order=workorder, order=current_process.order + 1
        ).first()
    ...
    @staticmethod
    def _auto_assign_next_tasks(
        workorder: WorkOrder, next_process: WorkOrderProcess
    ) -> None:
        """自动分配下一个工序的任务"""
        # 这里可以集成智能分配算法
        # 暂时简单分配给部门的操作员
        try:
            ...
            tasks = WorkOrderTask.objects.filter(
                workorder=workorder, process=next_process.process, status="ready"
            )
...
class ProcessBusinessService:
    ...
    def complete_process(
        workorder_process: WorkOrderProcess, completed_by: User, comments: str = ""
    ) -> bool:
        """
        完成工序
    ...
    @staticmethod
    def _complete_process_tasks(
        workorder_process: WorkOrderProcess, completed_by: User
    ) -> None:
        """完成工序相关任务"""
        tasks = WorkOrderTask.objects.filter(
    ...
    def get_workorder_statistics(
        ...
        for priority, label in [
            ...
            priority_stats[priority] = {
                ...
                "count": count,
                "percentage": (
                    round(count / total_orders * 100, 2) if total_orders > 0 else 0
                ),
            }

        # 金额统计
        total_amount = (
            queryset.aggregate(total=models.Sum("total_amount"))["total"] or 0
        )

        # 按月统计
        monthly_stats = (
            queryset.extra({"month": models.ExtractMonth("created_at")})
            .values("month")
            .annotate(count=models.Count("id"), amount=models.Sum("total_amount"))
            .order_by("month")
        )
    ...
    def get_task_statistics(
        ...
        for status, label in [
            ...
            status_stats[status] = {
                ...
                "count": count,
                "percentage": (
                    round(count / total_tasks * 100, 2) if total_tasks > 0 else 0
                ),
            }

        # 按用户统计
        user_stats = (
            queryset.values("assigned_to__username")
            .annotate(
