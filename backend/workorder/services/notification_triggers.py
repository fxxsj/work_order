...
def task_assigned_handler(sender, instance, created, **kwargs):
    ...
    if instance.assigned_operator:
        notification_service.notify_task_assigned(
            ...
            assigned_by=(
                instance.updated_by if hasattr(instance, "updated_by") else None
            ),
        )
                    ...
                    notification_service.send_notification(
                        ...
                        recipients=(
                            [
                                ...
                                instance.work_order_process.work_order.created_by,
                            ]
                            if instance.assigned_operator
                            else [instance.work_order_process.work_order.created_by]
                        ),
                        data={
                            "title": "任务开始执行",
        ...
        if instance.action_type == "completed":
            ...
            try:
                workorder_process = WorkOrderProcess.objects.get(
                    workorder=instance.work_order, process=instance.process
                )
            ...
            notification_service.send_notification(
                ...
                data={
                    ...
                    "created_by": (
                        instance.created_by.username if instance.created_by else ""
                    ),
                },
        ...
        if instance.action_type == "error":
            ...
            notification_service.send_notification(
                ...
                data={
                    ...
                    "created_by": (
                        instance.operator.username if instance.operator else ""
                    ),
                },
                priority=NotificationPriority.HIGH,
            )
...
class DeadlineWarningService:
    ...
    def check_deadline_warnings():
        ...
        workorders_1_day = WorkOrder.objects.filter(
            deadline=one_day_later, status__in=["pending", "in_progress"]
        )
        ...
        workorders_3_days = WorkOrder.objects.filter(
            deadline=three_days_later, status__in=["pending", "in_progress"]
        )
...
def _get_supervisors():
    """获取主管列表"""
    try:
        return User.objects.filter(groups__name="supervisor", is_active=True).distinct()
    except Exception:
        return []
