...
class StockConsistencyService:
    ...
    def validate_stock_consistency(product) -> Dict[str, any]:
        ...
        if difference != 0:
            issues.append(
                f"库存数量不一致：实际{actual_quantity}，预期{expected_quantity}，差异{difference}"
            )
    ...
    def fix_stock_consistency(product, user=None) -> Dict[str, any]:
        ...
        if validation["is_consistent"]:
            return {"success": True, "message": "库存数据一致，无需修复", "changes": []}
...
class WorkOrderQuantityService:
    ...
    def validate_quantity_consistency(work_order) -> Dict[str, any]:
        ...
        return {"is_consistent": len(issues) == 0, "summary": summary, "issues": issues}
...
class DataConsistencyManager:
    ...
    def _check_quantity_consistency() -> Dict[str, any]:
        ...
        for work_order in work_orders:
            validation = WorkOrderQuantityService.validate_quantity_consistency(
                work_order
            )
            if not validation["is_consistent"]:
        ...
        for work_order in work_orders:
            ...
            if not availability["is_available"]:
                material_issues.append(
                    {"work_order": work_order, "issues": availability["issues"]}
                )
            ...
            if not validation["is_consistent"]:
                try:
                    ...
                    if result["success"]:
                        fixed_count += 1
                        fix_details.append(
                            {"product_code": product.code, "changes": result["changes"]}
                        )
                    else:
                        failed_count += 1
