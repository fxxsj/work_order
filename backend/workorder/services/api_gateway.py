...
class WorkOrderAPIService:
    ...
    def update_workorder(
        workorder_id: int, data: Dict[str, Any], user: User
    ) -> Response:
        """更新施工单API"""
        try:
            workorder = WorkOrder.objects.get(id=workorder_id)
            updated_workorder = WorkOrderBusinessService.update_workorder(
                workorder, data, user
            )
            return APIResponse.success(
                data={
    ...
    def get_workorder_list(
        filters: Dict[str, Any] = None, user: User = None
    ) -> Response:
        """获取施工单列表API"""
    ...
    def approve_workorder(
        ...
        try:
            ...
            success = WorkOrderBusinessService.approve_workorder(
                workorder, user, comments
            )
    ...
    def reject_workorder(
        ...
        try:
            ...
            success = WorkOrderBusinessService.reject_workorder(
                workorder, user, comments
            )
    ...
    def _build_workorder_detail(workorder: WorkOrder) -> Dict[str, Any]:
        ...
        # 工序信息
        processes = WorkOrderProcess.objects.filter(work_order=workorder).order_by(
            "order"
        )
        detail["processes"] = [
            {
        ...
        detail["tasks"] = [
            {
                ...
                "process": {"id": task.process.id, "name": task.process.name},
...
class TaskAPIService:
    ...
    def assign_task(task_id: int, data: Dict[str, Any], user: User) -> Response:
        ...
        try:
            ...
            if success:
                ...
                return APIResponse.success(
                    data={"task_id": task.id, "assigned_to": assigned_to.username},
                    message="任务分配成功",
                )
            else:
    ...
    def start_task(task_id: int, user: User) -> Response:
        ...
        try:
            ...
            if success:
                return APIResponse.success(
                    data={"task_id": task.id, "started_at": task.started_at},
                    message="任务开始成功",
                )
            else:
...
class SystemAPIService:
    ...
    def health_check() -> Response:
        ...
        try:
            ...
            return APIResponse.error(message="系统异常", code=503, errors=[str(e)])
