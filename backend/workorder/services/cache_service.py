    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                logger.debug(
                    f"Cache set for key: {cache_key}, timeout: {cache_timeout}"
                )
                return result
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                logger.debug(
                    f"QuerySet cache set for key: {cache_key}, count: {len(ids)}"
                )
                return queryset
            except Exception as e:
...
def invalidate_cache_pattern(pattern: str) -> None:
    ...
    try:
        ...
        if cache_keys:
            ...
            logger.info(
                f"Invalidated {len(cache_keys)} cache keys matching pattern: {pattern}"
            )
    except Exception as e:
...
def set_cache_with_timeout(key: str, value: Any, timeout_name: str = "MEDIUM") -> None:
    ...
    """
    try:
        timeout = settings.CACHE_TIMEOUTS.get(
            timeout_name, settings.CACHE_TIMEOUTS["MEDIUM"]
        )
...
class CacheManager:
    ...
    @staticmethod
    def cache_user_data(
        user_id: int, data_key: str, data: Any, timeout: int = None
    ) -> None:
        """缓存用户相关数据"""
    ...
    @staticmethod
    def cache_workorder_data(
        workorder_id: int, data_key: str, data: Any, timeout: int = None
    ) -> None:
        """缓存施工单相关数据"""
    ...
    def get_workorder_data(
        workorder_id: int, data_key: str, default: Any = None
    ) -> Any:
        """获取施工单缓存数据"""
        cache_key = f"workorder_data:{workorder_id}:{data_key}"
        return get_cached_or_none(cache_key, default)
