...
class RealtimeNotificationService:
    ...
    def send_notification(
        ...
        # 发送邮件通知（如果是高优先级或紧急）
        if NotificationChannel.EMAIL in channels and priority in [
        ...
        try:
            ...
            for recipient in recipients:
                ...
                html_message = render_to_string(
                    ...
                    {"user": recipient, "notification": data, "data": data["data"]},
                )
    ...
    def notify_task_assigned(self, task, assigned_operator, assigned_by=None):
        ...
        # 获取工序名称
        process_name = (
            task.work_order_process.process.name
            if task.work_order_process
            else task.process_code
        )
    ...
    def _get_supervisors(self):
        ...
        try:
            ...
            return User.objects.filter(
                profile__role__in=["supervisor", "manager"], is_active=True
            ).distinct()
        except Exception:
            # Fallback to group-based check
            try:
                return User.objects.filter(
                    groups__name__in=["supervisor", "manager"], is_active=True
                ).distinct()
            except Exception:
                return []
    ...
    def _get_next_process(self, workorder, current_process):
        """获取下一个工序"""
        try:
            from ..models.core import Process, WorkOrderProcess
    ...
    def _get_process_operators(self, process):
        ...
        try:
            ...
            return User.objects.filter(
                userprofile__department=process.department, userprofile__is_active=True
            ).distinct()
        except Exception:
            return []
    ...
    def _get_workorder_operators(self, workorder):
        ...
        try:
            ...
            return User.objects.filter(
                userprofile__department__in=departments, userprofile__is_active=True
            ).distinct()
        except Exception:
            return []
