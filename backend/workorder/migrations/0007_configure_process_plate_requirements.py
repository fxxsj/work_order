...
def configure_process_plate_requirements_forward(apps, schema_editor):
    ...
    Process.objects.filter(code="PRT").update(
        requires_artwork=True, artwork_required=True
    )

    # 模切工序：需要刀模
    Process.objects.filter(code="DIE").update(requires_die=True, die_required=True)
...
def configure_process_plate_requirements_backward(apps, schema_editor):
    ...
    # 重置所有相关字段为默认值 False
    Process.objects.filter(code="PRT").update(
        requires_artwork=False, artwork_required=True  # 默认值就是 True，但这里明确设置
    )

    Process.objects.filter(code="DIE").update(requires_die=False, die_required=True)

    Process.objects.filter(code="FOIL_G").update(
        requires_foiling_plate=False, foiling_plate_required=True
    )
    ...
    Process.objects.filter(code="EMB").update(
        requires_embossing_plate=False, embossing_plate_required=True
    )
