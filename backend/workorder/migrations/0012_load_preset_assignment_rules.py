...
def load_preset_assignment_rules(apps, schema_editor):
    ...
    for rule_data in PRESET_ASSIGNMENT_RULES:
        ...
        existing_rule = TaskAssignmentRule.objects.filter(
            process=process, department=department
        ).first()

        if existing_rule:
            # 更新现有规则
            existing_rule.priority = rule_data["priority"]
            existing_rule.operator_selection_strategy = rule_data[
            ...
            existing_rule.save()
            print(
                f"更新规则: {process.name} -> {department.name} (优先级: {rule_data['priority']})"
            )
        else:
            # 创建新规则
            ...
            created_count += 1
            print(
                f"创建规则: {process.name} -> {department.name} (优先级: {rule_data['priority']})"
            )

    print(
        f"分派规则加载完成: 创建 {created_count} 条，更新 {TaskAssignmentRule.objects.count() - created_count} 条，跳过 {skipped_count} 条"
    )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            load_preset_assignment_rules, reverse_load_preset_assignment_rules
        ),
    ]
