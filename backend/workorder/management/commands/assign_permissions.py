...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(
                f'组 "{group_name}" 不存在。请先运行: python manage.py init_groups'
            )
        ...
        self.stdout.write(
            f'  组: {", ".join(user.groups.values_list("name", flat=True))}'
        )
        self.stdout.write(
            f"  权限数量: {user.user_permissions.count() + sum(g.permissions.count() for g in user.groups.all())}"
        )
        ...
        if group.permissions.exists():
            ...
            self.stdout.write("")
            self.stdout.write(self.style.WARNING(f'组 "{group_name}" 没有任何权限'))
