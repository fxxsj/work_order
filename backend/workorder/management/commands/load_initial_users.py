...
from workorder.models import Department, UserProfile
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if existing_users.exists() and not force:
            self.stdout.write(
                self.style.WARNING(f"⚠ 发现 {existing_users.count()} 个用户已存在：")
            )
        ...
        try:
            ...
            call_command(
                "loaddata", "workorder/fixtures/initial_users.json", verbosity=0
            )
