...
class Command(BaseCommand):
    ...
    def test_task_notification(self, user_id=None):
        ...
        try:
            ...
            if created_task:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"✓ 任务分配通知已发送给 {user.username}")
                )
            else:
    ...
    def test_workorder_notification(self, user_id=None):
        ...
        notification_service.notify_workorder_approval(
            workorder=workorder, status="approved", approved_by=workorder.created_by
        )
    ...
    def test_deadline_notification(self, user_id=None):
        ...
        if not workorder:
            self.stdout.write(
                self.style.WARNING("没有找到即将到期的施工单，创建一个测试用例...")
            )
