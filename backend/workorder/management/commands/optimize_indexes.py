...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--dry-run", action="store_true", help="仅显示将要创建的索引，不实际执行"
        )

        parser.add_argument("--analyze", action="store_true", help="分析现有查询性能")
        ...
        with transaction.atomic():
            ...
            for index_info in indexes_to_create:
                try:
                    ...
                    self.stdout.write(
                        f"✗ 创建索引失败: {index_info['table']}.{index_info['index']} - {str(e)}"
                    )
