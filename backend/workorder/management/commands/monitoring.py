...
class Command(BaseCommand):
    ...
    def health_check(self, options):
        ...
        if options.get("output"):
            ...
            self.stdout.write(
                self.style.SUCCESS(f'健康检查结果已输出到: {options["output"]}')
            )
    ...
    def performance_check(self, options):
        ...
        if stats["slowest_endpoints"]:
            ...
            for i, endpoint in enumerate(stats["slowest_endpoints"][:5], 1):
                self.stdout.write(
                    f"  {i}. {endpoint['name']}: {endpoint['execution_time']:.3f}s"
                )
    ...
    def business_metrics(self, options):
        ...
        self.stdout.write(
            f"  完成率: {workorder_metrics['order_stats']['completion_rate']:.1f}%"
        )
        self.stdout.write(
            f"  审核通过率: {workorder_metrics['approval_stats']['approval_rate']:.1f}%"
        )
        self.stdout.write(
            f"  平均完成天数: {workorder_metrics['time_metrics']['avg_completion_days']:.1f}天"
        )
        ...
        self.stdout.write(
            f"  CPU使用率: {system_metrics['system_resources']['cpu_percent']:.1f}%"
        )
        self.stdout.write(
            f"  内存使用率: {system_metrics['system_resources']['memory_percent']:.1f}%"
        )
        self.stdout.write(
            f"  磁盘使用率: {system_metrics['system_resources']['disk_percent']:.1f}%"
        )
