...
class CommonBehaviorMixin:
    ...
    def _login(self):
        ...
        user_type = self.user_type or random.choice(["supervisor", "operator", "maker"])
    ...
    def _get_random_task_id(self):
        ...
        response = self.client.get(
            "/api/workorder-tasks/?page_size=1", name="[Tasks] Get random ID"
        )
        if response.status_code == 200:
    ...
    def _get_random_workorder_id(self):
        ...
        response = self.client.get(
            "/api/workorders/?page_size=1", name="[WorkOrders] Get random ID"
        )
        if response.status_code == 200:
            results = response.json().get("results", [])
            if results:
...
class WorkOrderUser(CommonBehaviorMixin, HttpUser):
    ...
    def view_statistics(self):
        ...
        self.client.get(
            "/api/workorder-tasks/collaboration_stats/", name="[Stats] Collaboration"
        )
...
class SupervisorUser(CommonBehaviorMixin, HttpUser):
    ...
    def view_dashboard(self):
        ...
        self.client.get(
            "/api/workorder-tasks/collaboration_stats/", name="[Supervisor] Dashboard"
        )
