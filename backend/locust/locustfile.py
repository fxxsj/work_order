...
class CommonBehaviorMixin:
    ...
    def _get_random_task_id(self):
        ...
        response = self.client.get(
            "/api/tasks/?page_size=1", name="[Tasks] Get random ID"
        )
        if response.status_code == 200:
...
class SupervisorUser(CommonBehaviorMixin, HttpUser):
    ...
    def view_department_workload(self):
        ...
        self.client.get(
            "/api/statistics/department_workload/", name="[Supervisor] Workload"
        )
...
class OperatorUser(CommonBehaviorMixin, HttpUser):
    ...
    def view_my_tasks(self):
        ...
        self.client.get(
            "/api/tasks/", params={"status": "in_progress"}, name="[Operator] My Tasks"
        )
    ...
    def update_task_progress(self):
        ...
        if task_id:
            self.client.post(
                f"/api/tasks/{task_id}/update_progress/",
    ...
    def claim_task(self):
        ...
        if response.status_code == 200:
            ...
            if tasks:
                task_id = tasks[0]["id"]
                self.client.post(
                    f"/api/tasks/{task_id}/claim/", name="[Operator] Claim Task"
                )
