...
3. **前端添加页面**
   - 在 `apps/web/src/views/` 中创建页面组件
   - 在 `apps/web/src/router/` 中添加路由
   - 在 `apps/web/src/api/` 中添加 API 调用方法
...
- [用户操作指南](docs/USER_GUIDE.md) / [Admin 指南](docs/ADMIN_GUIDE.md)

## 标准项目通常包含哪些文档？

以下是一个“标准项目”常见的文档清单（本仓库已尽量对齐，细节见 `docs/README.md`）：

- `README.md`：项目概述、快速上手、常用入口
- `docs/`：可演进的详细文档（架构、运维、开发约定）
- `CHANGELOG.md`：版本级变更记录
- `LICENSE`：许可证文本
- `CONTRIBUTING.md`：贡献/协作规范
- `SECURITY.md`：安全漏洞披露与处理方式（可选但推荐）
...
| 文件 | 说明 |
|------|------|
| [PROJECT.md](.planning/PROJECT.md) | 项目上下文和核心价值定义 |
| [v1.0-REQUIREMENTS.md](.planning/milestones/v1.0-REQUIREMENTS.md) | 需求清单和验收标准 |
| [ROADMAP.md](.planning/ROADMAP.md) | 10 阶段路线图和进度追踪 |
| [STATE.md](.planning/STATE.md) | 当前项目状态和执行上下文 |
